...
function read_args(    arg) {
  ...
  if (!Afn || !Bfn || !OUTFILE || ARGC == 1 || ((FUZZY) && (ALLFUZZY))) {
    print "Usage:    matchnames -a query_names_file -b " \
      "ref_names_file -o outfile \\\n" \
      "            [ -f | -F [ -e fuzz_error ]] [ -1 -c -n -q ] [-l aregex]"
    exit 1
  }
...
function read_data(filename,     i, nameN, namex, rc) {

  while ((rc = (getline < filename)) > 0) {
    # clean fields of spaces and NULL symbols
    for (i = 1; i <= NF; i++) {
      gsub(/^(NULL|\\N)$/,"",$i)
      $i = cleanspaces($i)
    }
    if(rc<0) err_exit(1,ERRNO)
    ...
    if (++CodeN[$I_code] > 1) {
      print "ID code '" $I_code "' is a duplicate..."
      exit 1
    }
...
function test_fuzzy(code, lineno,   i, j, r, rc, found, nfound, mantype) {
  ...
  for (i in Gh[Bfn][G[code]]) {
    if (do_match(X[Bfn,"punct",i], X[Afn,"punct",code]))
      found[++nfound] = i
  }
...
function do_match(i1,i2) {
  if("amatch" in FUNCTAB)
    return amatch(i1, "^" i2 "$") > 0
  else
    return (levenshtein(i1,i2) <= FUZZERR)
}

function err_exit(rc, msg) {
  printf("matchnames: error(%d): %s\n", rc, msg) > "/dev/stderr"
  exit(_exit=rc)
}
